...
def extract_reasoning_and_text(
    ...
    # For reasoning in the 'reason' field
    if "reason" in data:
...
def format_as_markdown(reasoning: str | None, text: str | None) -> str:
    ...
    if reasoning:
        markdown += "## Reasoning\n\n"
        # Format reasoning as a markdown quote
        reasoning_lines = reasoning.split("\n")
        quoted_reasoning = "\n".join([f"> {line}" for line in reasoning_lines])
        markdown += quoted_reasoning + "\n\n"
...
def handle_sse_stream(response):
    ...
    panel = Panel(Markdown("## Waiting for response..."))
        ...
        try:
            for line in response.iter_lines():
                ...
                if line.startswith("event:"):
                    ...
                    # Update the display with the current event type using markdown
                    event_markdown = f"## Event: {event_type}\n\n"
                    if len(accumulated_reasoning) > 0:
                        event_markdown += "**Reasoning:** Available\n\n"
                    else:
                        event_markdown += "**Reasoning:** Not yet available\n\n"

                    if accumulated_text:
                        event_markdown += f"**Text Preview:** {accumulated_text[:50]}..."
                    else:
                        event_markdown += "**Text:** Not yet available"

                    panel = Panel(Markdown(event_markdown))
                    live.update(panel)
                    continue
                ...
                if data_str.startswith("{") or data_str.startswith("["):
                    try:
                        ...
                        markdown_content = format_as_markdown(accumulated_reasoning, accumulated_text)
                        # Update the display with the formatted markdown
                        panel = Panel(Markdown(markdown_content))
                        live.update(panel)
                        ...
                        if current_event_type == "response.completed":
                            ...
                            # Show the final response with markdown formatting
                            final_markdown = format_as_markdown(accumulated_reasoning, accumulated_text)
                            panel = Panel(Markdown(final_markdown))
                            live.update(panel)
            ...
            panel = Panel(Markdown(error_markdown))
            live.update(panel)
...
def send_hello_request(query=None, effort="low"):
    ...
    try:
        ...
        # If we want to fetch the response by ID later, uncomment these lines
        # if response_id:

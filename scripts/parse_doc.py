    ...
    async def upload_file(self, file_path: str, purpose: str = "general"):
        ...
        async with httpx.AsyncClient() as client:
            with open(path, "rb") as f:
                ...
                response = await client.post(f"{self.base_url}/v1/files", files=files, data=data)
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Parse a document using Knowledge Forge API and dump to JSON")
    parser.add_argument("-f", "--file", required=True, help="Path to the file to parse")

...
def ask_question_with_file_search(vector_store_id, question, server_url=local_url, effort="medium"):
    """Ask a question about the document using file search tools.

    Args:
        vector_store_id (str): ID of the vector store containing the document.
    ...
    try:
        ...
        print(f"Using file search with vector store ID: {vector_store_id}")

        # Prepare request payload with file_search tool
        payload = {
            "model": "qwen-max",  # Using the default model
            "tools": [
                {
                    "type": "file_search",
            ...
            "input": question,
            "effort": effort,  # Using specified effort level
        }
        ...
        # For SSE responses, we need to process the stream
                    ...
                    while "\n\n" in buffer:
                        ...
                        if event_type and data:
                            try:
                                ...
                                print(f"Error parsing event data: {data}")
...
def run_simple_flow(file_path, collection_name, question=None, server_url=local_url, effort="medium"):
    ...
    Args:
        ...
        question (str, optional): Question to ask about the document. If None, no question will be asked.
        server_url (str, optional): Server URL. Defaults to "http://localhost:9999".
        effort (str): Effort level for the response (low, medium, high).
    ...
    # Step 4: Ask a question about the document if provided
    if question:
        print("\n=== STEP 4: ASKING QUESTION ABOUT THE DOCUMENT ===")
        success = ask_question_with_file_search(vector_store_id, question, server_url, effort)
        if not success:
            print("Failed to get answer to the question.")
            return False
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-q",
        "--question",
        help="Question to ask about the uploaded document",
    )
    parser.add_argument(
        "--effort",
        "-e",
        ...
        default="medium",
        help="Effort level for the response (default: medium)",
    )

    args = parser.parse_args()

    # Run the simple flow
    run_simple_flow(args.file, args.name, args.question, args.server, args.effort)

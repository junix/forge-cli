...
class TraceableToolCall(BaseModel):
    ...
    progress: Optional[float] = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Current execution progress (0.0-1.0)"
    )
    
    execution_trace: Optional[str] = Field(
        default=None,
        description="Execution history as newline-separated log entries"
    )
    
    def capture_streaming_state(self, state: "ToolExecutionState") -> None:
        """Capture a streaming execution state into the tool call.
        
        This method updates the progress field and appends a timestamped
        message to the execution trace.
        
        Args:
            state: A ToolExecutionState object containing the current
                  execution status, progress, and message.
        """
        # Avoid circular import
        from tools.tool_execution_state import ToolExecutionState
        ...
        trace_line = f"[{timestamp}] {state.message}"
        
        # Add step name if available
        if state.step_name:
            trace_line = f"[{timestamp}] [{state.step_name}] {state.message}"
    ...
    def append_trace(self, message: str, step_name: Optional[str] = None) -> None:

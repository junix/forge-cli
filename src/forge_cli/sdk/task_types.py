...
class TaskStatus(BaseModel):
    ...
    type: str = Field(description="The type of operation the task is performing (e.g., 'file_processing', 'vector_store_creation').")
    status: Literal["pending", "in_progress", "completed", "failed", "cancelled"] = Field(description="The current status of the task.")
    ...
    progress: Optional[float] = Field(None, description="Task completion progress (e.g., percentage).", ge=0, le=100)
    result: Optional[dict[str, Any]] = Field(None, description="The result of the task if completed successfully. Structure may vary based on task type.")
    ...
    resource_id: Optional[str] = Field(None, description="Identifier of the primary resource associated with this task (e.g., file_id, vector_store_id).")
